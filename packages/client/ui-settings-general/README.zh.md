...
宿主端在用户设置 seam 中注册 `ui-onboarding`。`ui-settings-models` 提供的欢迎步骤通过既有公开 settings 边界读写其中的 `welcomeNoticeVersion`；外壳本身仍不持有产品策略。当 Tauri 桌面壳注入 `window.__DSH_DESKTOP__` 时，本插件还会注册通用设置里的**检查更新**行（order 90）；普通浏览器 `dsh web` 看不到该行。该行调用桥上的 `checkUpdate()` / `installUpdate()`，从不写入 session log。
...
- 浏览器 `dsh web` 下「通用」分区没有内置行。桌面壳注入 `window.__DSH_DESKTOP__` 后，本插件才会注册「检查更新」行。其他行仅在其所属功能插件挂载时出现。
