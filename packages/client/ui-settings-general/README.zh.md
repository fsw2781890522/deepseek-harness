...
宿主端在用户设置 seam 中注册 `ui-onboarding`。`ui-settings-models` 提供的欢迎步骤通过既有公开 settings 边界读写其中的 `welcomeNoticeVersion`；外壳本身仍不持有产品策略。只要存在 `settingsScope`，通用分区就会注册**代理端口**行（order 80）；它写入 `http-proxy.port`（默认 7897），供进程 HTTP 和桌面更新器使用，并不启用或禁用智能体工具。当 Tauri 桌面壳注入 `window.__DSH_DESKTOP__` 时，本插件还会注册通用设置里的**检查更新**行（order 90）；普通浏览器 `dsh web` 看不到该行。更新行调用桥上的 `checkUpdate()` / `installUpdate()`，从不写入 session log。
...
- 只要组合提供 `settingsScope`，通用分区就会出现**代理端口**行。「检查更新」行仅在桌面壳注入 `window.__DSH_DESKTOP__` 时出现。其他行仅在其所属功能插件挂载时出现。
