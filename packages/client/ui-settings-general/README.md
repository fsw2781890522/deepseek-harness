...
The Host half registers `ui-onboarding` in the user-settings seam. The welcome step contributed by `ui-settings-models` reads and writes its `welcomeNoticeVersion` through the existing public settings boundary; the shell itself remains policy-free. The General section always registers the **Proxy port** row (order 80) once `settingsScope` is present; it writes `http-proxy.port` (default 7897) for process HTTP and the desktop updater, and does not enable or disable agent tools. When the Tauri desktop shell injects `window.__DSH_DESKTOP__`, this plugin also registers the General **Check for updates** row (order 90); a plain `dsh web` browser does not see that row. The update row calls `checkUpdate()` / `installUpdate()` on that bridge and never talks to the session log.
...
- The General **Proxy port** row appears in every composition that provides `settingsScope`. The Check for updates row appears only when the desktop shell injects `window.__DSH_DESKTOP__`. Other rows appear only when their owning feature plugin is mounted.
