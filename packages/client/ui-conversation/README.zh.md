...
后续回复或已结束的 Session 封口一段连续的 Think、Tool、workflow 行之后，ChatView 把它们收进折叠的 `已处理 {duration}` / `Processed {duration}` 展开项。时长格式为 `x h x m x s`，省略前导零单位。仍在进行的尾部 run 在封口前保持展开；点击标题在本地 React state 中切换被包裹的 Seat（[决策](../../../.agents/notes/implemented/feature/2026-08-16-chat-process-run-collapse.md)）。

Chat 瀑布流左侧有一条 sticky 小地图，把当前已加载窗口里的每条 `user` Node 画成一根短横线。它停在对话滚动容器的左侧空隙，不随 transcript 滚动。悬停或键盘聚焦会弹出截断后的 prompt 卡片（首行、其余正文，以及存在时的图片名）；点击则把对应用户行滚到阅读区顶部。steering 与上下文行不进入该列表。未加载进当前事件窗口的用户消息没有对应短横线（[决策](../../../.agents/notes/implemented/feature/2026-08-16-chat-prompt-minimap.md)）。
