...
describe('ui-theme host', () => {
  it('registers, validates, and disposes the durable theme namespace with its fiber', async () => {
    ...
    expect(ctx.settings.get(ns)).toEqual({ preference: DEFAULT_PREFERENCE, sidebarTransparency: 40 })
    await ctx.settings.update(ns, { preference: 'dark' })
    expect(ctx.settings.get(ns)).toEqual({ preference: 'dark', sidebarTransparency: 40 })
