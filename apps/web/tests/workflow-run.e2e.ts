...
import {
  connectFreshWorkspace, expandProcessedGroups, newEnglishPage, REPO_ROOT, saveFailureShot,
} from './support.ts'
...
describe.skipIf(MODE === 'record')('web e2e: durable workflow run in Chat', () => {
  ...
  it('shows the live member, opens its local child, then retains the settled record beside the tool row', async () => {
    ...
    await settled
    await page.locator('[data-chat-process-group]').first().waitFor({ timeout: 15_000 })
